...
### configuring docker 
1. docker build -f DockerFile.dev .  // use -f to specify which file to use // . = build context important 
2. run function 
3. after successfully built container = 8cf522827a9d
4. docker run + conatiner id - to check container
5. run command docker build -f DockerFile.dev . + docker run + conatiner id for client / server / worker
6. configure yml file 
7. go to docker hub to get latest version of images + pull images + and start postgres databse on local machine
8. cmd: docker-compose up to pull lates images from docker hub and run localy
9. configure volumes 
- /app/node_modules = stays the same dont touch node modules 
            - ./server:/app = when accessing app user server directory
10. when server starts up pull info from environment e.g. process.env 
11. build + run 2 step process 
12. env vriable inside docker compose = only use it at run time only when itis started up 
13. variableName=value = sets var in conatiner at run time
14. ports can change so for routing - we specify /api to direct to correct service
15. after it goes thorugh nginx /api gets chopped off 
16. client: 3000 / server:5000 = actual names in nginx
17. port 80 inside container - connect outside with port mapping 
18. server may be a reserved word in nginx
19. proxy_pass http://client/ = send request to client upstream
20. rewrite /api/(.*) /$1 break; = using regex to slash off api - whatever matched is replaced by $1 = break = directive 
21. get defaul.conf into nginx
22. create docker file for nginx
23. COPY ./default.conf /etc/nginx/conf.d/ = take it and overwrite
24. worker crash is ok - nginx has to be running
25. expected <block end>, but found '<block mapping start>' = image: 'postgres:latest'  dont have to use latest tag
26. website for checking yml - https://onlineyamltools.com/validate-yaml
27. WARNING: Image for service nginx was built because it did not already exist. To rebuild this image you must use `docker-compose build` or `docker-compose up --build`.
28. app running on localhost 3050
29. ERROR: Service 'client' failed to build: The command '/bin/sh -c npm install' returned a non-zero code: 1 = firewall error 
try: You can try this solution noted here:

https://development.robinwinslow.uk/2016/06/23/fix-docker-networking-dns/

Also some solutions noted here:

https://github.com/npm/npm/issues/16661

30. after changing nginx config file - need to rebuild container 
